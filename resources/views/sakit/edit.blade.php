...
<div class="row" style="margin-top: 80px;">
    <div class="col">
        <form method="POST" action="/izinsakit/{{ $datasakit->kode_izin }}/update" id="formizin" enctype="multipart/form-data">
            @csrf
        <div class="form-group">
            <input type="text" id="tgl_izin_dari" value="{{ $datasakit->tanggal }}" name="tgl_izin_dari" class="form-control datepicker" placeholder="Dari">
                </div>
                <div class="form-group">
                    <input type="text" id="tgl_izin_sampai" value="{{ $datasakit->tgl_izin_sampai }}" name="tgl_izin_sampai" class="form-control datepicker" placeholder="Sampai">
                        </div>
                        <div class="form-group">
                             <input type="text" id="jml_hari" name="jml_hari" class="form-control" placeholder="Jumlah Hari" autocomplete="off" readonly>
                                </div>
                <div class="form-group">
                    <input type="text" id="keterangan" value="{{ $datasakit->keterangan }}" name="keterangan" class="form-control" placeholder="Keterangan" autocomplete="off"> 
                    </div>
            </div>
            ...
            @if ($datasakit->SID != null)
            <div class="row">
                <div class="col-12">
                    @php
                        $sid = Storage::url('/uploads/sid/'.$datasakit->SID ); // Corrected typo in 'uploads'
                    @endphp
                    <img src="{{ url($sid) }}" alt="" width="100px">
                </div>
            </div>
        @endif
            <div class="form-group">
